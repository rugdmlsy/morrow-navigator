...
Remote connections appear in the sidebar under **REMOTE** with explicit **SSH** or **GITHUB** type badges. **+** opens a connection picker populated from `~/.ssh/config`, showing each unused alias together with its detected type and effective endpoint metadata. The same picker provides **New SSH…** for creating a normal OpenSSH `Host` block from structured alias/host/user/port/identity-file fields. Navigator never stores passwords or private-key contents; new SSH connections are written to `~/.ssh/config` and authentication remains owned by the system SSH client. The minus button removes only the Navigator shortcut, not the underlying SSH config entry.
