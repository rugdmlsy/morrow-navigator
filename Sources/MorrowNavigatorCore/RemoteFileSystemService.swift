...
public enum RemoteHostKind: String, Sendable, Codable, CaseIterable {
    case ssh
    case github
...
public struct SSHConfigDiscovery: Sendable {
    ...
    public func hosts(configURL: URL? = nil) -> [RemoteHost] {
        ...
        var visited = Set<URL>()
        guard let text = expandedConfigText(at: url, visited: &visited) else {
            return []
        }
        ...
        for rawLine in text.split(omittingEmptySubsequences: false, whereSeparator: \.isNewline) {
            ...
            if fields.count >= 2, fields[0].lowercased() == "include" {
                for rawPath in fields.dropFirst() {
                    ...
                    guard !unquoted.contains("*"), !unquoted.contains("?"), !unquoted.contains("[") else {
                        continue
                    }
